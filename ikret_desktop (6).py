...
class PatientDashboard(tk.Frame):
    ...
    def _on_appt_select(self, event=None):
        ...
        if not hasattr(self, "appt_qr_id_label"):
            self.appt_qr_id_label = tk.Label(self.appt_qr_panel, text="", font=F_MONO_B,
                                              fg=TEAL_DARK, bg=PAPER_RAISED, wraplength=190,
                                              justify="left")
            self.appt_qr_id_label.pack(anchor="w", pady=(8, 0))
        self.appt_qr_id_label.config(text=f"Booking ID: {appt.get('id', '')}")
    ...
    def _book(self):
        ...
        messagebox.showinfo(
            ...
            f"Slot:        {slot}\n\n"
            f"Keep your Booking ID handy — it's shown under the QR\n"
            f"code below and in 'My Appointments' any time."
        )
    ...
    def _show_booking_qr(self, appt):
        ...
        tk.Label(self.qr_frame, text=f"Booking ID: {appt.get('id', '')}",
                 font=F_MONO_B, fg=TEAL_DARK, bg=PAPER_RAISED, wraplength=280,
                 justify="left").pack(anchor="w", pady=(6, 0))
        tk.Label(self.qr_frame, text=f"Token: {appt.get('token', '')}",
                 font=F_MONO, fg=INK_SOFT, bg=PAPER_RAISED, wraplength=280,
